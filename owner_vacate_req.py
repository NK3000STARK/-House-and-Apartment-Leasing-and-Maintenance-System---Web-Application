...
q1 = """select * from purchased_property where owner_email = '%s' and vacate_status = 'request'""" % owemail
cur.execute(q1)
rec1 = cur.fetchall()
...
for i in rec1:
    print("""
                                        ...
                                        <tr>
                                         <form method="post" enctype="multipart/form-data">
                                              <td><input name ="fid" value= "%s" style = "border:none;width:12px " readonly></td>
                                              <td><input name ="pname" value= "%s" style = "border:none " readonly></td>
                                              <td>%s</td>
                                              <td>%s</td>
                                              <td>%s</td>
                                              <td>%s</td>
                                              ...
                                              <input type = "text"name ="uemail" value= "%s" style = "border:none;display:none " readonly>
                                              <input type = "number" name = "balance" value = "%s" style = "border:none;display:none " readonly>
                                             </tr>
                                        </form>
                                     </div>
                                </div>
    ...
    </html>""" % (i[0], i[1], i[27], i[4], i[7], i[12], i[13], i[17],i[6],i[32]))
...
if Approve != None:
    newbal = int(balance) + 1

    today = date.today()
    q = """update purchased_property set vacate_date = '%s', vacancies = 'vacant' , remaining_bhk = %s, vacate_status = 'Approve', vacancies = 'vacant' where id ='%s' """%(today,newbal,fid)
    cur.execute(q)
    con.commit()
    q1 = """update property set vacancies = 'vacant', remaining_bhk = '%s' where propertyName = '%s' """% (newbal,pname)
    cur.execute(q1)
    con.commit()
    q3 = """update payment set vacate_status = 'vacant' where id = '%s'"""% fid
    cur.execute(q3)
    con.commit()
    fromadd = "snandhu712@gmail.com"
    password = "rpqdkhnufllixgtq"
    toadd = uemail
    subject = "Property Vacation Related"
    body = "Your property is vacated :{}".format(pname)
    msg = "Subject : {} \n\n {} ".format(subject, body)
    ...
    print("""
                ...
                location.href="owner_vacate_req.py?id=%s"
                </script>
               """% userid)

if Reject != None:
    q = """update property set vacancies = 'occupied' where propertyName = '%s' """ % (pname)
    cur.execute(q)
    con.commit()
    q1 = """update purchased_property set vacancies = 'occupied' where propertyName = '%s' """ % (pname)
    cur.execute(q1)
    con.commit()
    fromadd = "snandhu712@gmail.com"
    password = "rpqdkhnufllixgtq"
    toadd = uemail
    subject = "Property Vacation Related"
    body = "Your property vacate request is rejected by house owner :{}".format(pname)
    msg = "Subject : {} \n\n {} ".format(subject, body)
    server = smtplib.SMTP("smtp.gmail.com:587")
    server.ehlo()
    server.starttls()
