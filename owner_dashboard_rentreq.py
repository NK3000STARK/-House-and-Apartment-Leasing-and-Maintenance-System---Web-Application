...
q1 = """select * from purchased_property where owner_email = '%s' and status = 'request'""" % owemail
cur.execute(q1)
rec1 = cur.fetchall()
...
for i in rec1:
    ...
    </html>""" % (i[0],i[1],i[2],i[4],i[7],i[12],i[13],i[17]))
...
if Approve != None:
    q = """update purchased_property set status = '%s', vacancies = 'occupied' where id = '%s' """%(Approve,fid)
    cur.execute(q)
    con.commit()
    q1 = """update payment set property_status = 'Approve' where propertyName = '%s'"""% (pname)
    cur.execute(q1)
    con.commit()
    print("""
                <script>
                location.href="owner_dashboard_rentreq.py?id=%s"
                </script>
                """% userid)

if Reject != None:
    q = """update property set vacancies = 'vacant' where propertyName = '%s' """%(pname)
    cur.execute(q)
    con.commit()
    q1 = """update purchased_property set vacancies = 'vacant', status = 'new' where id = '%s' """%(fid)
    cur.execute(q1)
    con.commit()
    print("""
                <script>
